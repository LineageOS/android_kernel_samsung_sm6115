...
static struct fsm_config g_fsm_config = {
	...
	.i2s_srate  = 48000,
	.test_type  = TEST_NONE,
//ExtB P200626-05620, baoshulin@wingtech.com, 20200720, modify bootup default angle.
	.cur_angle  = 270,
	.next_angle = 270,
...
void fsm_firmware_show(uint16_t id)
{
    ...
    int chipid = id >>8;
    int chipver = id & 0xff;
    if(0x06 == chipid && chipver > 0)
    {
        snprintf(fsm_name,sizeof(fsm_name),"FS1818U");
        hardwareinfo_set_prop(HARDWARE_SMARTPA, fsm_name);
        return;
    }
    pr_err("%s fsm id:0x%x  chipid:0x%x chipver:0x%x",__func__,id,chipid,chipver);
    hardwareinfo_set_prop(HARDWARE_SMARTPA,"unknown");
}
EXPORT_SYMBOL(fsm_firmware_show);
				...
				int *re25, int *count)
{
	...
	if (re25 != NULL) {
		...
		//bug 576847 , baoshulin@wingtech.com, 20200723, solve pa write otp overflow
		if (byte == 0x7F) {
			byte = 0x00;
		}
...
int fsm_flip_angle()
{
	...
	if(cfg->next_angle >= 180) return 2;
	if(cfg->next_angle >= 90) return 1;
	if(cfg->next_angle >= 0) return 0;
	return 0;
}
...
void fsm_stereo_flip(int next_angle)
{
	...
	if (cfg->cur_angle == next_angle) {
		pr_info("cur_angle:%d, angle:%d", cfg->cur_angle, next_angle);
		return;
	}
	fsm_mutex_lock();
	pr_info("scene:%04X, angle:%d", cfg->next_scene, next_angle);
	cfg->next_angle = next_angle;
//ExtB611022, wangchengqiang.wt, modify for angle switch fail
	cfg->cur_angle = cfg->next_angle;
	if (cfg->stream_muted || !(cfg->next_scene & FSM_SCENE_MUSIC)) {
		fsm_mutex_unlock();
		pr_info("Not do swich !!!");
		return;
	}
