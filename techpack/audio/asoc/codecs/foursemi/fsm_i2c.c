...
static int fsm_request_irq(fsm_dev_t *fsm_dev)
{
	...
	ret = devm_request_threaded_irq(&i2c->dev, fsm_dev->irq_id,
				NULL, fsm_irq_hander, irq_flags, "fs18xx", fsm_dev);
	if (ret) {
			...
			const struct i2c_device_id *id)
{
    ...
    else{ //bug 538153, baoshulin@wingtech.com, 20200310, add mmitest and smartpa info
		pr_info("%s audiock fsm detect device seccess\n",__func__);
		smartpa_type =snd_soc_set_smartpa_type("fsm_i2c_probe",FS18XX);
	}
	...
	pr_addr(info, "audiock index:%d", fsm_dev->id);
	...
	dev_info(&i2c->dev, "audiock i2c probe completed");
	pr_info("audiock fsm %s\n end",__func__);
	return 0;
}
