...
static ssize_t fsm_angle_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "angle mode:%d\n", fsm_flip_angle());
}

static ssize_t fsm_angle_store(struct class *class,
				struct class_attribute *attr, const char *buf, size_t len)
{
	int ret =0;
	u8 mode = 0;
	printk("%s audiock buf :%s  last angle:%d\n",__func__,buf,fsm_flip_angle());
...
static struct class_attribute g_fsm_class_attrs[] = {
	...
	__ATTR(angle, S_IRUGO|S_IWUSR, fsm_angle_show, fsm_angle_store),
	__ATTR_NULL
};
...
static CLASS_ATTR_RW(fsm_angle);
